<?php
	...
	?>
				...
				<tr>
					...
					<td><a href="<?= site_url('../assets/files/edc_booking/'.$this->session->userdata('id').'/'.$app_detail['file_path']) ?>">Click to view Approval Letter</a></td>
				</tr>
				<? } ?>
		<?php
	...
	if(count($guest_details)!=0){
		...
		?>
								<tr>
									...
									<td><?=$i++?></td>
									<td><a href="<?= site_url('edc_booking/guest_details/show_guest_details/'.$user_id.'/'.$app_detail['app_num'].'/'.$guest['name'].'/'.$guest['check_in']) ?>"><?=$guest['name']?></a></td>
									<td><?=$guest['no_of_guests']?></td>
									<td><? foreach($guest['rooms'] as $room)
												if($room != 'Room Info Unavailable!')
													echo $room.'<br/>';
										   		else echo $room.'<br/>'; ?></td>
									<td><a href="<?= site_url('../assets/files/edc_booking/'.$user_id.'/'.$guest['identity_card']) ?>">View ID</a></td>
	...
	if($count_guest != $no_of_guests)
	{
		...
		<div class="tab-content">
			<div id="group" class="tab-pane fade in active">
				<? $form = $ui->form()->multipart()->action('edc_booking/guest_details/insert_guest/')->open();
			...
			echo '</div><div id="individual" class="tab-pane fade">';
				...
				$form = $ui->form()->multipart()->action('edc_booking/guest_details/insert_guest/')->open();
				//$form = $ui->form()->multipart()->action('edc_booking/guest_details/check/')->open();
